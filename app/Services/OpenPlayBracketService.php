...
class OpenPlayBracketService
{
     ...
     */
    public function generate(OpenPlaySession $event): ?string
    {
     ...
     * @return string|null An error message, or null on success.
     */
    public function addManualMatch(OpenPlaySession $event, OpenPlayRegistration $entry1, OpenPlayRegistration $entry2): ?string
    {
    ...
    private function generateRoundRobin(OpenPlaySession $event, $registrationIds): void
    {
        $now = now();
        $matches = [];
        ...
        OpenPlayMatch::query()->insert($matches);
    }

    private function generateSingleElimination(OpenPlaySession $event, $registrationIds): void
    {
    ...
    private function generateDoubleElimination(OpenPlaySession $event, $registrationIds): void
    {
        $seeds = $registrationIds->values();
        $entryCount = $seeds->count();
        $bracketSize = 2 ** (int) ceil(log($entryCount, 2));
    ...
    public function advanceWinner(OpenPlayMatch $match): void
    {
        $event = $match->openPlaySession;
    ...
    private function advanceSingleEliminationWinner(OpenPlayMatch $match): void
    {
        if ($match->winner_registration_id === null || $match->bracket_position === null) {
            return;
        }

        $nextMatch = OpenPlayMatch::query()
    ...
    private function advanceDoubleEliminationWinner(OpenPlayMatch $match): void
    {
        if ($match->winner_registration_id === null || $match->bracket_position === null) {
            return;
        }
    ...
    private function advanceLosersBracketMatch(OpenPlayMatch $match): void
    {
        $winnerId = $match->winner_registration_id;
        ...
        $nextLbMatch = OpenPlayMatch::query()
            ->where('open_play_session_id', $match->open_play_session_id)
            ->where('bracket_side', BracketSide::Losers)
    ...
    private function advanceFinalMatch(OpenPlayMatch $match): void
    {
        if ($match->round !== 1) {
        ...
        $resetExists = OpenPlayMatch::query()
            ->where('open_play_session_id', $match->open_play_session_id)
            ->where('bracket_side', BracketSide::Final)
            ->where('round', 2)
            ->exists();
        ...
        OpenPlayMatch::query()->create([
            'open_play_session_id' => $match->open_play_session_id,
    ...
    private function maybeAutoCompleteLosersBye(OpenPlayMatch $lbMatch): void
    {
    ...
    private function isLosersFeederPermanentlyDead(OpenPlayMatch $lbMatch, string $emptySlot): bool
    {
        if ($lbMatch->round === 1) {
            ...
            $wbMatch = OpenPlayMatch::query()
                ->where('open_play_session_id', $lbMatch->open_play_session_id)
                ->where('bracket_side', BracketSide::Winners)
                ->where('round', 1)

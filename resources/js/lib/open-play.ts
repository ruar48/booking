...
export function entryLabel(entry?: OpenPlayRegistration | null): string {
    if (!entry) {
        return '—';
    }
...
export function bracketSlotLabel(match: OpenPlayMatch, slot: 'entry1' | 'entry2'): string {
...
export function groupByRound(matches: OpenPlayMatch[]): Map<number, OpenPlayMatch[]> {
    const rounds = new Map<number, OpenPlayMatch[]>();
...
export function matchRoundLabel(match: OpenPlayMatch, matches: OpenPlayMatch[]): string {
...
export function matchesByBracketSide(matches: OpenPlayMatch[]): {
    winners: OpenPlayMatch[];
    losers: OpenPlayMatch[];
    final: OpenPlayMatch[];
} {
    const winners: OpenPlayMatch[] = [];
    const losers: OpenPlayMatch[] = [];
    const final: OpenPlayMatch[] = [];
...
export function losersBracketRoundHeights(matches: OpenPlayMatch[]): Map<number, number> {
...
export function isEntryInMatch(match: OpenPlayMatch, registrationId: number): boolean {

...
export default function AnnouncementsEdit({ announcement, openPlaySessions }: Props) {
    ...
    const { data, setData, put, processing, errors } = useForm<{
        ...
        open_play_session_id: announcement.open_play_session_id ?? null,
        image_mode: announcement.image_source === 'auto_qr' ? 'auto_qr' : announcement.image_path ? 'upload' : 'none',
        image: null,
    ...
    const submit = (event: FormEvent) => {
        ...
        put(update(announcement).url, { forceFormData: true });
    };
                ...
                <form onSubmit={submit} className="mx-auto w-full max-w-2xl space-y-6">
                    ...
                    <Card>
                        <CardContent className="grid gap-4 pt-6">
                                ...
                                <div className="grid gap-2">
                                    ...
                                    <div className="flex items-center gap-4">
                                        {(preview ?? announcement.image_url) && (
                                            ...
                                            <img
                                                src={preview ?? announcement.image_url ?? undefined}
                                                alt=""
                                        ...
                                        >
                                            {announcement.image_url ? 'Replace image' : 'Choose image'}
                                        </Button>
                                    </div>
                            ...
                            {data.image_mode === 'auto_qr' && (
                                <div className="flex items-center gap-4">
                                    {announcement.image_source === 'auto_qr' && announcement.image_url ? (
                                        // eslint-disable-next-line @next/next/no-img-element
                                        <img
                                            src={announcement.image_url}
                                            alt=""
                                            className="size-24 shrink-0 rounded-lg border object-contain p-2"
                                        />
                                    ) : joinUrl ? (

...
class OpenPlayDoubleEliminationSeeder extends Seeder
{
    ...
    {
        if (OpenPlaySession::where('title', 'Double Elimination Showdown')->exists()) {
            $this->command?->info('Double elimination demo session already seeded — skipping.');

            return;
        }

        $owner = User::where('email', 'owner@galaangramos.test')->first() ?? User::first();

        $members = Player::take(7)->get();

        if ($members->count() < 7) {
            $missing = 7 - $members->count();

            $members = $members->merge(
                Player::factory()->count($missing)->create()
            );
        }
